  ...
  <view class="container">
    ...
    <view class="start-section" v-if="!started">
      ...
      <view class="char-card" @click="goCharacter">
        ...
        <view class="char-info">
          ...
          <text class="char-name">{{ myChar ? myChar.name : '还没有创建，点击创建 ➜' }}</text>
        </view>
      </view>
    ...
    <scroll-view scroll-y class="story-scroll" v-if="started && !loading" :scroll-into-view="'bottom'">
      ...
      <view class="round-badge">
        <text class="round-text">第 {{ currentRound }} / {{ maxRounds }} 幕</text>
      </view>
      ...
      <!-- 选择分支 (当前轮) -->
      <view class="choices-section" v-if="!finished && currentChoices.length > 0">
      ...
      <view class="ending-section" v-if="finished">
        <view class="ending-card">
          ...
          <text class="ending-text">你完成了 {{ storyHistory.length }} 幕的冒险故事</text>
        </view>
...
.nav-content {
  ...
  padding: 0 30rpx 20rpx; height: 88rpx;
}
.nav-back {
...
.nav-link {
  ...
  padding: 10rpx 20rpx;
  background: rgba(255,255,255,0.8); border-radius: 30rpx;
  box-shadow: 0 2rpx 8rpx rgba(0,0,0,0.06);
}
.link-icon { font-size: 22rpx; }
.link-text { font-size: 22rpx; color: #7B4FE0; font-weight: bold; max-width: 120rpx; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }

/* 开始界面 */
.start-section { padding: 0 30rpx; display: flex; flex-direction: column; gap: 24rpx; }
.section-title { margin-bottom: 4rpx; }
.title-text { font-size: 34rpx; font-weight: bold; color: #3D2F5A; display: block; }
.title-desc { font-size: 24rpx; color: #8A7A9D; margin-top: 8rpx; }
...
.char-emoji { font-size: 44rpx; }
.char-info { flex: 1; }
.char-label { font-size: 22rpx; color: #8A7A9D; display: block; }
.char-name { font-size: 28rpx; font-weight: bold; color: #3D2F5A; }

.input-card {
  background: #fff; border-radius: 24rpx; padding: 28rpx 30rpx;
  box-shadow: 0 4rpx 20rpx rgba(0,0,0,0.05); border: 2rpx solid #E0D4F0;
}
.input-label { font-size: 26rpx; font-weight: bold; color: #7A6A8D; display: block; margin-bottom: 16rpx; }
.input-row { display: flex; align-items: center; gap: 16rpx; }
.keyword-input {
  flex: 1; background: #F6F2FC; border-radius: 16rpx;
  padding: 20rpx 24rpx; font-size: 28rpx; color: #3D2F5A;
...
.random-icon { font-size: 38rpx; }

.start-btn {
  background: linear-gradient(135deg, #9B6DFF, #7B4FE0);
  border-radius: 40rpx; padding: 34rpx; text-align: center;
  box-shadow: 0 8rpx 24rpx rgba(123,79,224,0.35);
}
.start-btn-text { font-size: 32rpx; font-weight: bold; color: #fff; }

/* 加载 */
.loading-section {
  padding: 100rpx 30rpx; display: flex; justify-content: center;
}
.loading-card {
  width: 100%; background: #fff; border-radius: 30rpx;
  padding: 80rpx 40rpx; display: flex; flex-direction: column;
  align-items: center; box-shadow: 0 8rpx 30rpx rgba(0,0,0,0.06);
  border: 2rpx solid #E0D4F0;
}
.loading-book { position: relative; width: 120rpx; height: 120rpx; margin-bottom: 40rpx; }
.book-emoji { font-size: 100rpx; }
.page-turn {
  position: absolute; width: 20rpx; height: 30rpx;
  background: #9B6DFF; border-radius: 4rpx; opacity: 0.3;
  animation: pageFlip 1.5s ease-in-out infinite;
}
.p1 { left: 30rpx; top: 20rpx; }
.p2 { left: 60rpx; top: 20rpx; animation-delay: 0.3s; }
@keyframes pageFlip {
  0%,100% { transform: rotateY(0deg); opacity: 0.3; }
  50% { transform: rotateY(180deg); opacity: 0; }
}
.loading-text { font-size: 32rpx; font-weight: bold; color: #3D2F5A; }
.loading-sub { font-size: 24rpx; color: #8A7A9D; margin-top: 12rpx; }

/* 故事场景 */
.story-scroll { padding: 0 30rpx; height: calc(100vh - 140rpx); box-sizing: border-box; }
.round-badge {
  display: flex; justify-content: center; margin: 16rpx 0;
}
.round-text {
  background: linear-gradient(135deg, #9B6DFF, #7B4FE0);
...
.scene-card {
  ...
  border: 2rpx solid #E0D4F0; position: relative;
  animation: fadeIn 0.4s ease-out;
}
.scene-last {
  border-color: #9B6DFF; box-shadow: 0 4rpx 20rpx rgba(123,79,224,0.1);
}
@keyframes fadeIn {
...
.scene-number {
  position: absolute; top: -14rpx; left: 24rpx;
  background: #9B6DFF; color: #fff; font-size: 20rpx; font-weight: bold;
  width: 40rpx; height: 28rpx; line-height: 28rpx; text-align: center;
  border-radius: 20rpx;
}
.scene-text { font-size: 28rpx; color: #3D2F5A; line-height: 1.7; }
.chosen-tag { margin-top: 12rpx; }
.chosen-text { font-size: 22rpx; color: #7B4FE0; font-weight: 600; }

/* 选择 */
.choices-section { margin: 24rpx 0; }
.choices-label { font-size: 26rpx; font-weight: bold; color: #7A6A8D; display: block; margin-bottom: 16rpx; }
.choice-btn {
  display: flex; align-items: center; gap: 16rpx;
  background: #fff; border: 2rpx solid #D0C0E8;
  border-radius: 20rpx; padding: 24rpx 20rpx;
  margin-bottom: 12rpx; transition: all 0.2s;
}
.choice-btn:active { transform: scale(0.98); background: #F6F2FC; }
.choice-index {
  width: 44rpx; height: 44rpx; line-height: 44rpx; text-align: center;
  background: #9B6DFF; color: #fff; font-weight: bold; font-size: 22rpx;
  border-radius: 50%; flex-shrink: 0;
}
.choice-text { flex: 1; font-size: 28rpx; color: #3D2F5A; }
.choice-arrow { font-size: 28rpx; color: #7B4FE0; }

/* 结局 */
.ending-section { margin: 24rpx 0; }
.ending-card {
  background: linear-gradient(135deg, #F8F4FF, #F0E8FF);
  border: 2rpx solid #D0C0E8; border-radius: 28rpx;
  padding: 40rpx; text-align: center;
}
.ending-emoji { font-size: 80rpx; display: block; margin-bottom: 16rpx; }
.ending-title { font-size: 36rpx; font-weight: bold; color: #3D2F5A; display: block; }
.ending-text { font-size: 24rpx; color: #8A7A9D; margin-top: 8rpx; display: block; }
.ending-actions { display: flex; gap: 16rpx; margin-top: 20rpx; }
.end-action {
  flex: 1; padding: 26rpx; text-align: center; border-radius: 40rpx;
  transition: all 0.2s;
}
.end-retry { background: #fff; border: 2rpx solid #9B6DFF; }
.end-save { background: linear-gradient(135deg, #9B6DFF, #7B4FE0); box-shadow: 0 4rpx 16rpx rgba(123,79,224,0.3); }
.end-text { font-size: 26rpx; font-weight: bold; }
.end-retry .end-text { color: #7B4FE0; }
.end-save .end-text { color: #fff; }
</style>